...
#'
#' @examples
#' Oman2 <- LoadOmanTESTING()
#' @seealso [LoadCountries()]
#' @export
LoadOmanTESTING <- function(tidy = TRUE, DaysOld = 30, minimumpercapitaactivecases = 0, RiskEval = NULL, dropNACountry = FALSE, dropNAall = FALSE) {
  ...
  Oman_DATA <- tidy_Data(Oman_DATA, tidy, DaysOld, minimumpercapitaactivecases, RiskEval, dropNACountry, dropNAall)
  
  return(Oman_DATA)
  }, warning = function(w){LoadErrorWarning("LoadOmanTESTING")}
