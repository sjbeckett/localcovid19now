...
Currently, updating country geometries is only intended to be done in interactive mode. This tutorial assumes use of a "Project" in RStudio, while it may be doable outside this environment.
...
2. Write a "process script" for the countries you want to update/add to the package. This can be done through either manually writing a new script or using the included "snippet."
    ...
    a. (Optional) Install the snippet.
        ...
        Open `RSnippets.txt`. In RStudio, go to Tools > Global Options > Code > Edit Snippets. Paste the contents of `RSnippets.txt` to the bottom of the "edit snippets" page. Make sure all indents are done using "tab" (they will automatically be highlighted red if not).
...
3. Run `library(subregionalcovid19)`
